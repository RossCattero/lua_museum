...
if (CLIENT) then
	...
	local function GetWeaponQuality(cond)
		local text, condcol = 0, color_black
		if (cond) then
			if cond >= 90 then
	...
	function ITEM:PopulateTooltip(tooltip)
		local qualitytext, qualitycolor = GetWeaponQuality(self:GetData('WeaponQuality', 100));
		if (self:GetData("equip")) then
			local name = tooltip:GetRow("name")
			name:SetBackgroundColor(derma.GetColor("Success", tooltip))
		end
		local row = tooltip:AddRowAfter("weight", "quality")
		row:SetText("Качество: "..qualitytext)
		row:SetTextColor(qualitycolor)
...
ITEM.functions.TakeDownAttributes = {
	name = "Снять модификации",
	OnRun = function(item)
		local ply = item.player;
		local weapon = ply:GetWeapon(item.class);
		local att = item:GetData('WepAttachments')
		
		if att["sight"] != "" then
			if ply:HasWeapon(item.class) then weapon:Detach(att["sight"]) end;
			ply:GetCharacter():GetInventory():Add(att["sight"])
			att["sight"] = "";
		end;
		if att["grip"] != "" then
			if ply:HasWeapon(item.class) then weapon:Detach(att["grip"]) end;
			ply:GetCharacter():GetInventory():Add(att["grip"])
			att["grip"] = "";
		end;
		if att["supressor"] != "" then
			if ply:HasWeapon(item.class) then weapon:Detach(att["supressor"]) end;
			ply:GetCharacter():GetInventory():Add(att["supressor"])
			att["supressor"] = "";
		end;
		if att["laser"] != "" then
			if ply:HasWeapon(item.class) then weapon:Detach(att["laser"]) end;
			ply:GetCharacter():GetInventory():Add(att["laser"])
			att["laser"] = "";
		end;
	...
	OnCanRun = function(item)
		return !IsValid(item.entity) && !IsWeaponMelee(item.class)
	end;
}
...
function ITEM:OnEquipWeapon( ply, wep )
	local data = self:GetData( "WepAttachments" )
...
function ITEM:OnInstanced()
	if !self:GetData('WepAttachments') then
		self:SetData('WepAttachments', {sight = "", grip = "", supressor = "", laser = ""})
	end;
	if !self:GetData( 'WeaponQuality' ) then
		self:SetData( 'WeaponQuality', 100 )
	end;
end;
