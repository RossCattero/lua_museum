...
function PANEL:Init()
    ...
    self.questType:AddChoice( "Истребление мутантов" )
    ...
    function self.listOfNeededButton:DoClick(self)
        ...
        if !pnl.listOfNeeded.needlist[pnl.listOfNeededIndex:GetValue()] then
            if pnl.questType:GetValue() == 'Собирательство' && !ix.item.list[pnl.listOfNeededIndex:GetValue()] then
                return;
            end;
    ...
    function self.rewardTaskButton:DoClick(self)
        ...
        if !pnl.rewardTask.rewardList[pnl.rewardTaskIndex:GetValue()] then
            if pnl.questType:GetValue() == 'Собирательство' && !ix.item.list[pnl.rewardTaskIndex:GetValue()] then
                return;
            end;
...
function PANEL:SaveAnwsers()
    ...
    netstream.Start('EditAnwsers', {self.ids, self.talkTable} );
end;
...
function PANEL:SaveTalkers()
    ...
    netstream.Start('EditEntityTalker', self.getent, tbl );
end;
