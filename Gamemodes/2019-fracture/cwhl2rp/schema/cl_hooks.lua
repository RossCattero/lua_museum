...
function Schema:GetCinematicIntroInfo()
	return {
		credits = "Сервер разработал "..self:GetAuthor()..".",
...
function Schema:ScoreboardSortClassPlayers(class, a, b)
	if (class == "Гражданская Оборона" or class == "Сверхчеловеческий отдел") then
...
function Schema:GetPlayerScoreboardClass(player)
	...
	--[[if (faction == FACTION_CITIZEN) then
		return "Гражданские";
	elseif (faction == FACTION_REFUGEE) then
		return "Беженцы";
	elseif (faction == FACTION_CWUW or faction == FACTION_CWUMED or faction == FACTION_CWUDIR) then
		return "Гражданский Союз Рабочих";
	elseif (faction == FACTION_RESISTANCE or faction == FACTION_RELEADER) then
		return "Сопротивление";
	elseif (faction == FACTION_VORT or faction == FACTION_VORTSLAVE or faction == FACTION_HOUNDEYE) then
		return "Существа из Зена";
	elseif (faction == FACTION_CW) then
		return "Дети-Рабочие";
	elseif (faction == FACTION_MPF) then
		return "Гражданское Управление Альянса";
	elseif (faction == FACTION_ADMIN) then
		return "Городская Администрация";
	elseif (faction == FACTION_CREMATOR) then
		return "Синтетические Войска Альянса";
	elseif (faction == FACTION_OTA) then
		return "Сверхчеловеческие Войска Альянса";
	end;]]
end;
...
function Schema:PlayerAdjustClassModelInfo(class, info)
	if (class == CLASS_MPS) then
		if (self:IsPlayerCombineRank(Clockwork.Client, "SCN")
		and self:IsPlayerCombineRank(Clockwork.Client, "SYNTH")) then
			info.model = "models/shield_scanner.mdl";
		else
			info.model = "models/combine_scanner.mdl";
		end;
	...
	if (IsValid(Clockwork.Client)) then
		if (self:PlayerIsCombine(Clockwork.Client) && Clockwork.Client:GetSharedVar("GasMaskInfo") >= 2) then
			...
			if (!self.nextRandomLine or curTime >= self.nextRandomLine) then
					...
					self.nextRandomLine = curTime + 15;
