...
hook.Add("HUDPaint", "Crosshair", function()
	if (GetConVar("evo_crosshair"):GetInt() == 1) then
		if (Schema:PlayerIsCombine(LocalPlayer())) then
	local x = ScrW() / 2
	local y = ScrH() / 2
...
hook.Add("HUDPaint", "General Interface", function()
	...
	for k,e in pairs ( player.GetAll() ) do
		if ( e:IsPlayer() and e != LocalPlayer() and !e:IsNoClipping() and e:GetPos():Distance(LocalPlayer():GetPos()) <= 500) then
