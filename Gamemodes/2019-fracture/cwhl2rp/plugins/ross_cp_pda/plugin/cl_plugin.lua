...
cable.receive('OpenCivilProtectionPDA', function(table, settings, ID, logs)
    OpenCPPDA_Password(table, settings, ID, logs)
end);

cable.receive('OpenRegistrationObject', function(table)
    RegObj(table)
end);
...
function RegObj(table)
	...
	for k, v in pairs(PLUGIN.CPchat) do
...
function OpenCPPDA_Password(table, Settings, ID, actualLogs)
    local scrW = surface.ScreenWidth();
	local scrH = surface.ScreenHeight();
	...
	Password.OnEnter = function()
        ...
        elseif Password:GetValue() == password then
            ...
            PasswordPanel:AlphaTo(0, 1, 0, function()
                ...
                OpenCPPDA(frame, OutLinedPanel, frame.everything, frame.settings, ID, actualLogs)
            end);
...
function OpenCPPDA(frame, OutLinedPanel, tableInformation, Settings, ID, actualLogs)
	for k, v in pairs(Settings["styleSettings"]) do
		if v["current"] == true then
			OutLinedPanel.ColorFrameBackground = v[1]
		end;
	end;
	...
	two.DoClick = function(self)
		if OutLinedPanel.getPageOfPDA != "chat" then
			OpenCPChat(OutLinedPanel, frame.everything, frame.settings, ID)
		end;
...
function OpenCPChat(OutLinedPanel, tableInformation, Settings, id)
	local panelsColor;
	for k, v in pairs(Settings["styleSettings"]) do
		if v["current"] == true then
			panelsColor = v[1]
	...
	for k, v in pairs(PLUGIN.CPchat) do
		...
		chateek.Paint = function(self, w, h)
			draw.RoundedBoxOutlined( 0, 0, 0, w, h, panelsColor, Color(255, 255, 255) )
			draw.DrawText( v["text"], "DermaDefault", 100, 4, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT )
	...
	ChatSend.OnEnter = function()
		...
		table.insert(PLUGIN.CPchat, {
			text = ChatSend:GetValue(),
			name = nameself,
			id = id
		})
		chatPanel:Remove()
		OpenCPChat(OutLinedPanel, tableInformation, Settings, id)
	end;
...
function OpenCPPDASettings(OutLinedPanel, tableInformation, Settings, ID)
	...
	local settingsInfo = vgui.Create( "DPanel", OutLinedPanel )
	settingsInfo:SetPos( 10, 25 )
	settingsInfo:SetSize( 750, 420 )
	settingsInfo.Paint = function(self, w, h)
        draw.RoundedBox(0, 0, 0, w, h, panelsColor);
	end;
	settingsInfo.Think = function(self)
		if OutLinedPanel.getPageOfPDA != "settings" then
			self:Remove();
		end;
	end;
	local settingsInfoI = vgui.Create( "DPanel", settingsInfo )
	settingsInfoI:SetPos( 5, 5 )
	settingsInfoI:SetSize( 740, 410 )
	settingsInfoI.Paint = function(self, w, h)
        draw.RoundedBoxOutlined( 0, 0, 0, w, h, panelsColor, Color(255, 255, 255) )
	end;
	local DThemeS = vgui.Create( "DComboBox", settingsInfoI )
	DThemeS:SetPos( 10, 10 )
	DThemeS:SetSize( 100, 20 )
	DThemeS:SetValue( currentTheme )
	for k, v in pairs(Settings["styleSettings"]) do
		DThemeS:AddChoice( k )
	end;
	DThemeS.OnSelect = function( self, i, b )
		for k, v in pairs(Settings["styleSettings"]) do
			if k == currentTheme && v["current"] == true then
				v["current"] = false
			end;
			if Settings["styleSettings"][b] != currentTheme && Settings["styleSettings"][b]["current"] == false then
				Settings["styleSettings"][b]["current"] = true;
			end;
		end;

		cable.send('EditUniversalSettingsPDA', Settings, ID);
	end	
