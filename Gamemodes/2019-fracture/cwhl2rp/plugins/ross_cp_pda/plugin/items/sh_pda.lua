...
function ITEM:OnUse(player, itemEntity)
    ...
    for k, v in pairs(cwPlayer.GetAll()) do
        ...
        if v:GetFaction() == FACTION_MPF then
            ...
            local one, two, three = string.find(v:GetName(), ".+(%d%d%d)");
            everyonedata[three] = {
                ["Name"] = v:GetName(),
    ...
    cable.send(player, 'OpenCivilProtectionPDA', everyonedata, self:GetData("SettingsData"), self.itemID, Schema.PDAlogs);
