...
if ( CLIENT ) then
	SWEP.DrawAmmo			= false
	SWEP.PrintName			= "Сверхспособности"
	--SWEP.Author				= "ss"
	SWEP.DrawCrosshair		= true
	SWEP.ViewModelFOV		= 54
	SWEP.Instructions = "Первичный огонь: Выстрел.\nВторичный: Лечить.";
	--SWEP.Contact = "";
	SWEP.Purpose = "Предмет защиты. Энергия сил из Зена.";
end
...
SWEP.LoweredAngles 			= Angle(0.000, 0.000, -15.000)
...
function SWEP:Think()
	...
	--if self.ArmorRegenTime < CurTime() and self.Owner:GetAmmoCount(self.Primary.Ammo) < 5 then
	--	self.ArmorRegenTime = CurTime() + 1
	--	self.Owner:GiveAmmo( 1, "AR2AltFire", true )
	--end
...
function SWEP:GiveArmor()
	...
	if plarm <= (self.ArmorLimit - arm) then
		...
		healety:SetHealth(self.ArmorLimit)
	end
...
function SWEP:SecondaryAttack()
	...
	if ((IsValid(trace.Entity) and trace.Entity:IsPlayer() and trace.Entity:Health() < self.ArmorLimit) or self.Owner:Health() < self.ArmorLimit) then
		if self.Owner:GetAmmoCount(self.Primary.Ammo)<self.AmmoPerUse  then 
			self.Weapon:EmitSound(self.Deny)
			self.Weapon:SetNextPrimaryFire(CurTime()+SoundDuration(self.Deny))
			self.Weapon:SetNextSecondaryFire(CurTime()+SoundDuration(self.Deny))
