...
cable.receive('ReturnItemToInventory', function(player, entity, data)
	...
	for k, v in pairs(entity.inventoryInside[data[1]]) do
		if v.uniqueID == data[2] && v.itemID == data[3] && (invw + Clockwork.item:FindByID(data[2]).weight) <= maxw then
			local placemodel = "";
			if v.cookingBlueprint[ data[4] ].model == nil then
				placemodel = v.model
			...
			item("data").DHunger = v.needsList.dhunger;
			item("data").DThirst = v.needsList.dthirst;
			item("data").DSleep = v.needsList.dsleep;
			item("data").AddDirt = v.needsList.addDirt;
	...
	Clockwork.player:EntityConditionTimer(player, entity, entity, timeToCreate, 192, function()
		...
		if (success) then
			if IsValid(entity) then
				for k, v in pairs(entity.inventoryInside[data[1]]) do
					-- (invw + Clockwork.item:FindByID(v.unqiueID).weight) <= maxw &&
					if v.IsCrafted == false then
						local placemodel = "";
						if v.cookingBlueprint[data[2]]["model"] == nil then
							placemodel = v.model
						...
						entity.inventoryInside[data[1]][k] = {
							...
							model = placemodel,
							needsList = {
								dhunger = v.needsList.dhunger + v.cookingBlueprint[data[2]].hun,
								dthirst = v.needsList.dthirst + v.cookingBlueprint[data[2]].thirst,
								dsleep = v.needsList.dsleep + v.cookingBlueprint[data[2]].sleep,
								addDirt = v.needsList.addDirt + v.cookingBlueprint[data[2]].dirt				
							},
							IsCrafted = true;
						};
					end;
