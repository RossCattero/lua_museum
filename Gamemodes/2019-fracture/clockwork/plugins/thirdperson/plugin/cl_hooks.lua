...
function cwThirdPerson:Initialize()
	...
	CW_CONVAR_CHASECAM_BACK = 55;
	CW_CONVAR_CHASECAM_RIGHT = 20;
	CW_CONVAR_CHASECAM_UP = 6;
...
function cwThirdPerson:CalcView(player, pos, angles, fov)
	...
	if (player:GetThirdPerson()) then
		...
		if (player:GetVar("thirdperson_zoom") != 1) then
			offset.x = CW_CONVAR_CHASECAM_BACK
			if (player:GetNWBool("ChaseCamInvert")) then
				local right = CW_CONVAR_CHASECAM_RIGHT;
				offset.y = right - (right * 2);
			else
				offset.y = CW_CONVAR_CHASECAM_RIGHT;
			end;

			offset.z = CW_CONVAR_CHASECAM_UP;
		end;
