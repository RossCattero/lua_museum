...
function PLUGIN:DoUnapprovedPlayerSpawn(player)
	-- Notify the player
	player:CPNotify("Добро пожаловать! Чтобы скачать контент, напишите /content", "new");
	-- Get list of admins
	...
	if (#listeners > 0) then
		...
		player:CPNotify("Если хотите поговорить с администратором, то просто напишите сообщение в /adminhelp.", "user_go");
	else
		-- Notify player no admins are currently one
		player:CPNotify("Нету администраторов в сети.", Clockwork.option:GetKey("wait_icon"));
	end;
		...
		command.OnRun = function(COMMAND, player, arguments)
			...
			if (!Clockwork.player:IsProtected(arguments[1])) then
				if (duration) then
						...
						if (IsValid(player)) then
							if (steamName) then
								if (duration > 0) then
									...
									if (hours >= 1) then
										Clockwork.player:NotifyAll(player:Name().." заблокировал '"..steamName.."' на "..hours.." час(ов) ("..reason..").");
									else
										Clockwork.player:NotifyAll(player:Name().." заблокировал '"..steamName.."' на "..math.Round(duration / 60).." минут(ы) ("..reason..").");
									end;
								else
									Clockwork.player:NotifyAll(player:Name().." заблокировал '"..steamName.."' навсегда ("..reason..").");
								end;
							else
					...
					Clockwork.player:Notify(player, "Не валидное количество!");
				end;
			else
				local target = Clockwork.player:FindByID(arguments[1]);
				
				if (target) then
					Clockwork.player:Notify(player, target:Name().." защищен!");
				else
					Clockwork.player:Notify(player, "Этот игрок защищен!");
				end;
			end;
		end;
	end;
	...
	if (command) then
		command.OnRun = function(COMMAND, player, arguments)
			...
			if (Clockwork.bans.stored[identifier]) then
				Clockwork.player:NotifyAll(player:Name().." разбанил '"..Clockwork.bans.stored[identifier].steamName.."'.");
				Clockwork.bans:Remove(identifier, nil, player);
			else
				Clockwork.player:Notify(player, "Нету забаненых игроков с идентификатором '"..identifier.."'!");
			end;
		end;
	end;
end;