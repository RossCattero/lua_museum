...
Clockwork.chatBox:RegisterClass("cp_warn_player", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, info.icon, PLUGIN.color, "[CP] ", "@ПРЕДУПРЕЖДЕНИЕ ", "от ", PLUGIN.color, info.speaker:SteamName(), ": ", info.text);
end);
Clockwork.chatBox:RegisterClass("cp_warn_admin", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, info.icon, PLUGIN.color, "[CP] ", _team.GetColor(info.speaker:Team()), info.speaker:SteamName(), " предупредил",
...
Clockwork.chatBox:RegisterClass("cp_deathnotification", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, "icon16/exclamation.png", PLUGIN.color, "[CP] ", info.data.attackerColor, info.data.attackerName, " убил ", info.data.playerColor, info.data.playerName, "!");
end);

-- Admin Approve message
Clockwork.chatBox:RegisterClass("cp_unapprovedplayer", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, "icon16/new.png", PLUGIN.color, "[CP] ", info.data.color, info.text, " (OOC: ", info.data.ooc, ") появился и не подтвержден.");
end);
...
Clockwork.chatBox:RegisterClass("cp_connect", nil, function(info)
	...
	Clockwork.chatBox:Add(filtered, "icon16/"..icon..".png", Color(34, 83, 83, 255), info.text.." подключился к серверу.");
end);
