...
COMMAND.text = "<string Name|SteamID|IPAddress> <number Hours> [string Reason]";
...
function COMMAND:OnRun(player, arguments)
	...
	if (!Clockwork.player:IsProtected(arguments[1])) then
		if (duration) then
			Clockwork.bans:Add(arguments[1], duration, reason, function(steamName, duration, reason)
				if (IsValid(player)) then
					if (steamName) then
						if (duration > 0) then
							local hours = math.Round(duration);
