...
function Clockwork:GetEntityMenuOptions(entity, options)
	...
	if (class == "cw_item") then
		...
		if (itemTable) then
			...
			options["Взять"] = "cwItemTake";
			-- options["Описать"] = "cwItemExamine";
		end;
	elseif (class == "cw_belongings") then
		options["Открыть"] = "cwBelongingsOpen";
	elseif (class == "cw_shipment") then
		options["Открыть"] = "cwShipmentOpen";
	elseif (class == "cw_cash") then
		options["Взять"] = "cwCashTake";
...
local SCREEN_DAMAGE_OVERLAY = cwKernel:GetMaterial("serveri/hardo.png");
...
local tab = {
	...
	surface.SetDrawColor(255, 255, 255, math.Clamp(255 * damageFraction, 0, 255)); -- damageFraction
	DrawColorModify(tab)
...
function Clockwork:HUDPaintForeground()
	...
	draw.RoundedBox( 0, x - 16, y - 16, width + 32, height + 32, Color( 0, 0, 0, 150 ) );
...
function Clockwork:GetScreenTextInfo()
	...
	if (cwClient:GetSharedVar("CharBanned")) then
		return {
			...
			title = "ЭТОТ ПЕРСОНАЖ ПОГИБ",
			text = "Перейдите в главное меню, чтобы создать нового."
		};
	end;
end;
					...
					elseif (trace.Entity:IsWeapon()) then
						if (cwClient:GetShootPos():Distance(trace.HitPos) <= fadeDistance) then
							...
							if (!active) then
								...
								y = cwKernel:DrawInfo("Неизвестное оружие.", x, y, Color(200, 100, 50, 255), alpha);
								y = cwKernel:DrawInfo("Нажмите 'ИСПОЛЬЗОВАТЬ', чтобы подобрать.", x, y, colorWhite, alpha);
							end;
						end;
...
function Clockwork:DrawTargetPlayerStatus(target, alpha, x, y)
	...
	if (!target:Alive()) then
		return cwKernel:DrawInfo(gender.." не имеет пульса.", x, y, informationColor, alpha);
	else
		return y;
	end;
end;
...
function Clockwork:GetStatusInfo(player, text)
	...
	if (action) then
		if (!player:IsRagdolled()) then
			if (action == "lock") then
				table.insert(text, "[Закрывает]");
			elseif (action == "unlock") then
				table.insert(text, "[Открывает]");
			end;
		elseif (action == "unragdoll") then
			if (player:GetRagdollState() == RAGDOLL_FALLENOVER) then
				table.insert(text, "[Встаёт]");
			else
				table.insert(text, "[Без сознания]");
			end;
		elseif (!player:Alive()) then
			table.insert(text, "[Мёртв]");
		else
			table.insert(text, "[Выполняет '"..action.."']");
		end;
	end;
...
-- Поломанная хуйня. Никогда не раскоменчивать. (c) Ross. 13.08.2018
--[[
	@codebase Client
...
function Clockwork:GetTargetPlayerText(player, targetPlayerText)
	...
	if (cwPly:DoesRecognise(player, RECOGNISE_PARTIAL)) then
		...
		for k, v in pairs(physDescTable) do
			targetPlayerText:Add("PHYSDESC_"..k, utf8.sub(v, 1, 38).."...");
		end;
	elseif (player:Alive()) then
		targetPlayerText:Add("PHYSDESC", "Вы не знаете "..thirdPerson..".");
	end;
end;
...
function Clockwork:GetPlayerScoreboardText(player)
	...
	if (cwPly:DoesRecognise(player, RECOGNISE_PARTIAL)) then
		...
		if (string.utf8len(physDesc) > 38) then
			return utf8.sub(physDesc, 1, 38).."...";
		else
			return utf8.sub(physDesc, 1, 38);
		end;
	else
		return "Вы не знаете "..thirdPerson..".";
	end;
end;
...
function Clockwork:HUDDrawScoreBoard()
	...
	if (self.ClockworkIntroFadeOut) then
		...
		if (curTime <= self.ClockworkIntroWhiteScreen) then
			...
			draw.RoundedBox( 0, 0, y - 8, ScrW, height + 16, Color( 0, 0, 0, 150 ) );
...
function Clockwork:PostDrawBackgroundBlurs()
	...
	if (self.TitledMenu and IsValid(self.TitledMenu.menuPanel)) then
		...
		draw.RoundedBox(2, menuPanel.x - 4, menuPanel.y - 4, menuPanel:GetWide() + 8, menuPanel:GetTall() + 8, backgroundColor)
...
concommand.Add("cwLua", function(player, command, arguments)
	...
	print("Вы не имеете доступа к этой команде, "..player:Name()..".");
end);
