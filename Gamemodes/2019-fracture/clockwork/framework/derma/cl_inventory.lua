...
function PANEL:Init()
	...
	self.columnSheet:AddSheet("Инвентарь", self.inventoryList);
	self.columnSheet:AddSheet("Снаряжение", self.equipmentList);
...
function PANEL:Rebuild()
	...
	self.weightForm:SetText("Вес", nil, "basic_form_highlight");
	...
	if (Clockwork.inventory:UseSpaceSystem()) then
		...
		self.spaceForm:SetText("Место", nil, "basic_form_highlight");
	...
	if (#categories.equipment > 0) then
		for k, v in pairs(categories.equipment) do
			...
			categoryForm.Paint = function( self, x, y )

	    		if not categoryForm then return end
...
function PANEL:Init()
	...
	self.spawnIcon:SetColor(Color(0, 0, 0, 0));
...
function PANEL:Init()
	...
	self.spawnIcon = Clockwork.kernel:CreateMarkupToolTip(vgui.Create("cwSpawnIcon", self));
	self.spawnIcon:SetColor(Color(0, 0, 0, 150));
...
function PANEL:Think()
	...
	self.spawnIcon:SetColor(Color(0, 0, 0, 0)); -- self.itemTable("color")
...
function PANEL:Paint(w, h)
	return draw.RoundedBox(4, 0, 0, w, h, Color(0, 0, 0, 150));
end;
...
function PANEL:Init()
	...
	function self.spaceUsed.Paint(spaceUsed)
        ...
        draw.RoundedBox(0, 0, 0, spaceUsed:GetWide(), spaceUsed:GetTall(), Color(25, 25, 25, 150));
        draw.RoundedBox(0, 0, 0, width, spaceUsed:GetTall(), Color(113, 161, 215, 255));
		--Clockwork.kernel:DrawSimpleGradientBox(0, 0, 0, spaceUsed:GetWide(), spaceUsed:GetTall(), color);
		--Clockwork.kernel:DrawSimpleGradientBox(0, 0, 0, width, spaceUsed:GetTall(), Color(113, 161, 215, 255));
	end;
end;
