...
function PANEL:Rebuild(change)
	if (!Clockwork.theme:Call("PreMainMenuRebuild", self)) then
		...
		if (IsValid(self.characterMenu)) then
		...
		for k, v in pairs(Clockwork.MenuItems.stored) do
			...
			if (Clockwork.menu.stored[v.panel]) then
				...
				panel:SetPos(self.tabX + (scrW - width - (scrW * 50)), self.tabY + (scrH * 10));
				panel.Name = v.text;
			end;
			...
			if (button) then
				...
				button:SetPos(x + 15, y);
				
				y = y + button:GetTall() - 20;
...
function PANEL:OpenPanel(panelToOpen)
	if (!Clockwork.theme:Call("PreMainMenuOpenPanel", self, panelToOpen)) then
		...
		if (CW_CONVAR_FADEPANEL:GetInt() == 1) then
			self.activePanel:SetPos(scrW - width - (scrW * 0.04 - 70), scrH * 0.1);
		...
		for k, v in pairs(Clockwork.menu:GetItems()) do
			if (IsValid(v.button)) then
				if (v.panel == activePanel) then
					...
					v.button.ColorToShow.red = 255;
					v.button.ColorToShow.green = 97;
					v.button.ColorToShow.blue = 0;
					v.button.ColorToShow["BorderColor"] = Color(255, 58, 0, 255);
				else
					v.button.LabelButton:OverrideTextColor(false);
					v.button.ColorToShow.red = 0;
					v.button.ColorToShow.green = 0;
					v.button.ColorToShow.blue = 0;
					v.button.ColorToShow["BorderColor"] = Color(100, 100, 100)
				end;
			end;
		end;
	end;
