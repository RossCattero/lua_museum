...
function PANEL:RebuildPanel(storagePanel, storageType, usedWeight, weight, usedSpace, space, cash, inventory)
	...
	cashForm.Paint = function(self, x, y)
		...
		button.Paint = function(self, x, y)
			draw.RoundedBox(4,0,0,x,y,Color(10, 10, 10, 150));
		end
		...
		cashForm:DockPadding(10, 10, 10, 10);
	...
	local informationForm = vgui.Create("DForm", storagePanel);
		informationForm:DockPadding(10,10,10,10)
		informationForm:SetSpacing(5);
		informationForm:SetName(L("Weight"));
	informationForm.Paint = function(self, x, y)
       	draw.RoundedBox( 0, 0, 0, x, y, Color( 0, 0, 0, 150 ) );
    end;
	...
	if (#categories > 0) then
		for k, v in pairs(categories) do
			...
			local categoryList = vgui.Create("DPanelList", collapsibleCategory);
				...
				categoryList:DockPadding(10,10,10,10);
...
vgui.Register("cwDFrame", PANEL, "DFrame");
...
Clockwork.datastream:Hook("StorageStart", function(data)
	...
	Clockwork.storage.panel = vgui.Create("cwDFrame");

Clockwork.storage.panel.Paint = function( self, w, h )
