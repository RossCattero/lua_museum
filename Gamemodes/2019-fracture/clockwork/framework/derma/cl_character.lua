...
function PANEL:ResetTextAndPositions()
	...
	self.titleLabel:SetPos((scrW * 0.6) - (self.titleLabel:GetWide() / 2), scrH * 0.88);
	self.subLabel:SetPos((scrW * 0.6) - (self.subLabel:GetWide() / 2), self.titleLabel.y + self.titleLabel:GetTall() + 8);
	self.titleLabel:SetAlpha(100)
	self.subLabel:SetAlpha(100)
...
function PANEL:Paint(w, h)
	...
	if (!Clockwork.theme:Call("PreCharacterMenuPaint", self)) then
		...
		if (schemaLogo != "" and subLabelAlpha > 0) then
			...
			surface.DrawTexturedRect(scrW * 0.02, 360, 512, 256);
		end;
		...
		if (Clockwork.character:IsCreationProcessActive()) then
			...
			draw.RoundedBox( 0, 0, progressY, scrW, progressHeight, boxColor );
			...
			draw.RoundedBox( 0, 0, progressY, (scrW / 100) * creationProgress, progressHeight, colorTargetID );
...
function PANEL:Init()
	...
	function self.pointsUsed.Paint(pointsUsed)
		...
		draw.RoundedBox( 0, 0, 0, pointsUsed:GetWide(), pointsUsed:GetTall(), colorTargetID );
		
		if (self.totalPoints > 0 and self.totalPoints < self.attributeTable.maximum) then
			draw.RoundedBox( 0, 0, 0, width, pointsUsed:GetTall(), colorTargetID );
		end;
	end;
end;
...
function PANEL:Init()
	...
	self.categoryList:SetSpacing(8);
	self.categoryList:SizeToContents(); 
	self.categoryList.backgroundHidden = true;	 
