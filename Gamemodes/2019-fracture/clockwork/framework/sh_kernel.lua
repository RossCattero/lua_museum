...
if (SERVER) then
	...
	function Clockwork.kernel:CalculatePlayerDamage(player, hitGroup, damageInfo)
		...
		if (player:Armor() > 0 and bDamageIsValid and bHitGroupIsValid) then
			...
			if (armor < 0) then
				Clockwork.limb:TakeDamage(player, hitGroup, damageInfo:GetDamage() * 2.5);
		...
		if (damageInfo:IsFallDamage()) then
			Clockwork.limb:TakeDamage(player, HITGROUP_RIGHTLEG, damageInfo:GetDamage() * 2.5);
			Clockwork.limb:TakeDamage(player, HITGROUP_LEFTLEG, damageInfo:GetDamage() * 2.5);
		end;
	end;
	...
	Clockwork.CinematicsWB = Clockwork.CinematicsWB or {};
	...
	function Clockwork.kernel:DrawMarkupToolTip(markupObject, x, y, alpha) -- !
		...
		--self:DrawSimpleGradientBox(2, x - 8, y - 8, width + 16, height + 16, Color(50, 50, 50, alpha));
			draw.RoundedBox( 2, x - 8, y - 8, width + 16, height + 16, Color( 0, 0, 0, 150 ) );
...
collapsibleCategory.Paint = function( self, x, y )
    ...
    --derma.SkinHook( "Paint", "CollapsibleCategory", self, x, y )
	...
	function Clockwork.kernel:DrawArmorBar()
		...
		if (armor > 0) then
			Clockwork.bars:Add("ARMOR", Color(139, 174, 179, 255), "Броня", self.armor, Clockwork.Client:GetMaxArmor(), self.health < 10, 1);
		end;
	end;
	...
	function Clockwork.kernel:DrawHealthBar()
		...
		if (health > 0) then
			Clockwork.bars:Add("HEALTH", Color(179, 46, 49, 255), "Здоровье", self.health, Clockwork.Client:GetMaxHealth(), self.health < 10, 2);
		end;
	end;
	...
	function Clockwork.kernel:DrawCinematicIntroBars()
		if (Clockwork.config:Get("draw_intro_bars"):Get()) then
			...
			draw.RoundedBox(0, 0, 0, ScrW(), Clockwork.CinematicScreenBarLength, Color(0, 0, 0, 0));
			draw.RoundedBox(0, 0, ScrH() - Clockwork.CinematicScreenBarLength, ScrW(), maxBarLength, Color(0, 0, 0, 0));
		end;
	end;
