...
function Clockwork:ClockworkSchemaLoaded() 
 if (tonumber(Clockwork.kernel:GetVersion()) >= 0.97) then
 	-- self.directory:AddCategoryPage("\"HelpCredits\"", "\"HelpClockwork\"", "\"http://authx.cloudsixteen.com/credits.php\"", false);
 	self.directory:AddPage("HelpBugsIssues", "http://github.com/CloudSixteen/Clockwork/issues", false);
 	self.directory:AddPage("HelpCloudSixteen", "http://forums.cloudsixteen.com", false);
 	self.directory:AddPage("HelpUpdates", "http://authx.cloudsixteen.com/updates.php", false);
 	else
 	-- self.directory:AddCategoryPage("Credits", "Clockwork", "http://authx.cloudsixteen.com/credits.php", false);
 	self.directory:AddPage("Bugs/Issues", "http://github.com/CloudSixteen/Clockwork/issues", false);
 	self.directory:AddPage("Cloud Sixteen", "http://forums.cloudsixteen.com", false);
 	self.directory:AddPage("Updates", "http://authx.cloudsixteen.com/updates.php", false);
 end;
end;
...
function Clockwork:PlayerDataLoaded(player)
	if (cwConfig:Get("clockwork_intro_enabled"):Get()) then
		if (!player:GetData("ClockworkIntro")) then
			cwDatastream:Start(player, "ClockworkIntro", false);
...
function Clockwork:Tick()
	...
	if (!self.NextSaveData or sysTime >= self.NextSaveData) then
		...
		self.NextSaveData = sysTime + 3600;
	end;
	
	if (!self.NextCheckEmpty) then
		self.NextCheckEmpty = sysTime + 3600;
	end;
	
	if (sysTime >= self.NextCheckEmpty) then
		self.NextCheckEmpty = nil;
		
		if (table.Count(plyTable) == 0) then
			cwPlugin:Call("PreSaveData");
			cwPlugin:Call("SaveData");
			cwPlugin:Call("PostSaveData");
		end;
...
function Clockwork:PlayerHealthRegenerate(player, health, maxHealth)
	...
	if (player:Alive() and (!player.cwNextHealthRegen or curTime >= player.cwNextHealthRegen)) then
		if (health >= (maxHealth / 2) and (health < maxHealth)) then
			...
			player.cwNextHealthRegen = curTime + 160;
		elseif (health > 0) then
			player:SetHealth(
				math.Clamp(health + 1, 0, maxHealth)
			);
...
function Clockwork:PlayerCanDeleteCharacter(player, character)
	if (cwConfig:Get("cash_enabled"):Get() and character.cash < cwConfig:Get("default_cash"):Get()) then
		if (!character.data["CharBanned"]) then
			return "Вы не можете удалить персонажа, у которого меньше чем "..cwKernel:FormatCash(cwConfig:Get("default_cash"):Get(), nil, true)..".";
		end;
	end;
end;
...
function Clockwork:PlayerCanSwitchCharacter(player, character)
	if (!player:Alive() and !player:IsCharacterMenuReset() and !player:GetSharedVar("CharBanned")) then
		return "Вы не можете переключаться между персонажами пока мертвы!";
	elseif (player:GetRagdollState() == RAGDOLL_KNOCKEDOUT) then
		return "Вы не можете переключаться между персонажами пока находитесь без сознания!";
	end;
...
function Clockwork:PlayerCanUseCharacter(player, character)
	if (character.data["CharBanned"]) then
		return character.name.." заблокирован и не может быть использован!";
	end;
...
function Clockwork:EntityHandleMenuOption(player, entity, option, arguments)
	...
	local antidupe = 1 + math.random(0, 1.35);
	
	if (class == "cw_item" and (arguments == "cwItemTake" or arguments == "cwItemUse")) then
		Clockwork.player:SetAction(player, "PickingUpItem", antidupe);
		Clockwork.player:EntityConditionTimer(player, entity, entity, antidupe, 192, function()
		...
		Clockwork.player:SetAction(player, "PickingUpItem", false);
	end);
...
function Clockwork:PlayerSpawnedProp(player, model, entity)
	if (IsValid(entity)) then
		...
		if (IsValid(entity)) then
			...
			if (IsValid(entity)) then
				cwKernel:PrintLog(LOGTYPE_URGENT, player:Name().." заспавнил '"..tostring(model).."'.");
...
--[[
	...
	@returns {Unknown}
--]]
function Clockwork:PrePlayerTakeDamage(player, attacker, inflictor, hitGroup, damageInfo) end;
...
function Clockwork:DoPlayerDeath(player, attacker, damageInfo)
	...
	player:StripAmmo();
	player:AddDeaths(1);
	--player:RunConsoleCommand("play", "usesound/death.mp3");
	--surface.PlaySound("usesound/death.mp3");
	player:UnLock();
...
function Clockwork:PlayerLoadout(player)
	...
	if (cwConfig:Get("give_keys"):Get() and player:GetFaction() != FACTION_VORT and player:GetFaction() != FACTION_OTA and player:GetFaction() != FACTION_OTA_SOW and player:GetFaction() != FACTION_CREMATOR) then
...
--[[
	@codebase Server
	@details Called when a player tdpresses F1.
	@param {Unknown} Missing description for player.
	@returns {Unknown}
--]]
