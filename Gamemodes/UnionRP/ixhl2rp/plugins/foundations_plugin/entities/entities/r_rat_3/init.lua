...
function ENT:Initialize()
	...
	function ENT:IncreaseItem(uniqueID, rationAdd)
		if self.ingredientsInside[uniqueID] then
			self.ingredientsInside[uniqueID] = mc(self.ingredientsInside[uniqueID] + (rationAdd or 1), 0, 32);
		end;
	end;
	...
	function ENT:StartTouch(ent)
		...
		if ent:GetClass() == 'ix_item' then
			...
			if ingr && ingr < 32 then
				self:IncreaseItem(itemTable.uniqueID, itemTable.rationAdd)
	...
	function ENT:Use(activator, caller)
		...
		if !self.SpawnItemCoolDown or self.SpawnItemCoolDown <= CurTime() then
			...
			self.ration:SetSkin(ix.item.list[self.lvltbl[lvl]].skin or 0)
			...
			timer.Simple(1.5, function()
				...
				ix.item.Spawn(self.lvltbl[lvl], self.ration:GetPos(), function(item, entity)
					self.ration:SetNoDraw(true)
