...
function Schema:RenderScreenspaceEffects()
	...
	if LocalPlayer():GetCharacter() then
		local data = LocalPlayer():GetLocalVar('ClothesUps', { gasmask = '' });

		if (LocalPlayer():IsCombine() && (data['gasmask'] != '' && data['gasmask'] != nil) ) then
...
function Schema:UpdateCharacterInfo(panel)
	...
	if !panel.animationsList then
		panel.animationsList = panel:Add("DComboBox")
		panel.animationsList:Dock(TOP)
	    panel.animationsList:DockMargin(0, 0, 0, 8)
    	panel.animationsList.Paint = function(_, w, h) 
	        surface.SetDrawColor(100, 100, 100, 255)
        	surface.DrawRect(0, 0, w, h)
		end

		for k, v in pairs(ix.act.stored) do
			panel.animationsList:AddChoice( k )
		end;

		panel.animationsList.OnSelect = function( self, index, value )
			netstream.Start('DoPriorityAction', 'Act'..value)
		end
	end;
end
