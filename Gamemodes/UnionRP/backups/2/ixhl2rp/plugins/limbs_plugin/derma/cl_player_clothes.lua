...
hook.Add("CreateMenuButtons", "ixClothes", function(tabs)

	tabs["Одежда"] = {
        bDefault = true,
        OnSelected = function(info, container)
            timer.Simple(0.2, function()
                if (PLUGIN.clothes and PLUGIN.clothes:IsValid()) then PLUGIN.clothes:Close(); end;
                PLUGIN.clothes = container:Add('PlayerClothes')
                PLUGIN.clothes:Populate()
            end)
        end,
        ...
        OnDeselected = function(info, container)
            if IsValid(PLUGIN.clothes) then
                PLUGIN.clothes:Close();
            end;
        end
	}
end)
