...
function ENT:Initialize()
	...
	timer.Create(self:EntIndex()..' - lootProp', self.informTable['timetoclean'], 0, function()
		if !IsValid(self) then
			timer.Remove(self:EntIndex()..' - lootProp')
			return;
		end;

		if table.Count(self.items) == 0 then return end;

		self.itemsInside = {};
	end)

	timer.Create(self:EntIndex()..' - lootProp_2', self.informTable['timetospawn'], 0, function()
		if !IsValid(self) then
			timer.Remove(self:EntIndex()..' - lootProp_2')
			return;
		end;
		if table.Count(self.items) == 0 then return end;
