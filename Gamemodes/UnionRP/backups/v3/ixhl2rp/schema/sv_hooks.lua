...
function Schema:PlayerUse(client, entity)
	...
	if (client:GetCharacter():GetInventory():HasItem('hard_lock_card') && entity:IsDoor() and IsValid(entity.ixLock) and client:KeyDown(IN_SPEED)) then
		entity.ixLock:Toggle(client)
		return false
	end

	if (client:GetCharacter():GetInventory():HasItem('union_lock_card') && entity:IsDoor() and IsValid(entity.ixLockUnion) and client:KeyDown(IN_SPEED)) then
		entity.ixLockUnion:Toggle(client)
		return false
	end
...
function Schema:PlayerUseDoor(client, door)
	if (client:IsCombine() or client:Team() == FACTION_PARTY_BUREAU) then
...
netstream.Hook("DoPriorityAction", function(client, action)
	ix.command.Run(client, action, {})
end)