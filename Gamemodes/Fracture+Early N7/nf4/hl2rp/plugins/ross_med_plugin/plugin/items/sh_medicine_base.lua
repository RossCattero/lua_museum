    ...
    function ITEM:OnCustomFunction(player, funcName)
        ...
        if funcName == "Применить к себе" then       
            ...
            if self('immunity') && self('immunity') > 0 then
                ...
                if player:GetCharacterData('Immunity') + self('immunity') == 200 then
            ...
            if !symsIsNil then
                for sympthom, chance in pairs(syms) do
                    if math.random(0, 100) - GetSkillValue(player, ATB_SUSPECTING) <= chance && player:HasSym(sympthom) then
        ...
        elseif funcName == "Применить к кому-то" then
            ...
            if EntTrace:IsPlayer() then
                ...
                if !symsIsNil then
                    for sympthom, chance in pairs(syms) do
                        if math.random(0, 100) - GetSkillValue(player, ATB_SUSPECTING) <= chance && EntTrace:HasSym(sympthom) then
