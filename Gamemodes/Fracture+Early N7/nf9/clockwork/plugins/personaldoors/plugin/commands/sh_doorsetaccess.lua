...
function COMMAND:OnRun(player, arguments)
	...
	if (IsValid(door) and Clockwork.entity:IsDoor(door)) then
		if (not door._OwningPersons or not door._OwningPersons[lowerName]) then
			...
			Clockwork.player:Notify(player, "Вы дали доступ '".. owningPerson .."' к этой двери.");
		else
			Clockwork.player:Notify(player, "Этот игрок уже имеет доступ к двери!");
		end;
	else
		Clockwork.player:Notify(player, "Это не дверь!");
	end;
end;
