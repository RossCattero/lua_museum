...
function PANEL:Init()
	...
	self.propertySheet.Paint = function(self, w, h)
	...
	self.sellsPanel.Paint = function(self, x, y)
       	draw.RoundedBox( 0, 0, 0, x, y, Color( 0, 0, 0, 150 ) );
    end;
		
		self.propertySheet:AddSheet("Продает", self.sellsPanel, "icon16/box.png", nil, nil, "Посмотреть предметы, которые "..salesmenuName.." продает.");
	end;
	...
	self.buysPanel.Paint = function(self, x, y)
		...
		self.propertySheet:AddSheet("Покупает", self.buysPanel, "icon16/add.png", nil, nil, "Посмотреть предметы, которые "..salesmenuName.." покупает.");
	end;
	...
	if (Clockwork.config:Get("cash_enabled"):Get()) then
		...
		if (totalCash > -1) then
			local cashForm = vgui.Create("DForm", panelList);
				...
				cashForm:DockPadding(0,4,0,4)
				cashForm.Paint = function(self, x, y)
       	        draw.RoundedBox( 0, 0, 0, x, y, Color( 255, 255, 255, 150 ) );
                end;	
			panelList:AddItem(cashForm);
			
			cashForm:Help(
				Clockwork.salesmenu:GetName().." имеет "..totalCash.." жетонов в наличии."
			);
		end;
	end;
	...
	local function DisplayCallback(displayInfo)
		...
		if (Clockwork.config:Get("cash_enabled"):Get()) then
			if (self.itemTable("cost") != 0) then
				...
				displayInfo.weight = L("Бесплатно");
			end;
