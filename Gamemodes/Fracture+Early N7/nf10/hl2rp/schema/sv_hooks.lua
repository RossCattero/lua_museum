...
function Schema:CheckPassword(steamID, ipAddress, svPassword, clPassword, name)
	...
	if !table.HasValue(self.white, steamID) then
...
function Schema:ClockworkInitPostEntity()
	...
	Schema.white = Clockwork.kernel:RestoreSchemaData("plugins/wl/wl");
end;
...
function Schema:PostSaveData()
	...
	Clockwork.kernel:SaveSchemaData("plugins/wl/wl", Schema.white);
end;

function Schema:Initialize()
	if !Clockwork.kernel:SchemaDataExists('plugins/wl/wl') then
		Clockwork.kernel:SaveSchemaData("plugins/wl/wl", Schema.white);
	else
		Schema.white = Clockwork.kernel:RestoreSchemaData("plugins/wl/wl");
	end;
