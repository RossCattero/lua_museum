...
function playerMeta:SetBodyGroups()
	...
	local bodyGroupsList = self:GetCharacterData("bgs");
	local bodyGroupListq = self:GetCharacterData("BodyGroups");
	...
	-- Set faction bodygroupsq
	if (faction.bodyGroupsListq) then
		PLUGIN:SetBodyGroupsFromTable(self, faction.bodyGroupsListq)
	end;
	...
	-- Set class bodygroupsq, overwrite faction bodygroups if needed
	if (class.bodyGroupsListq) then
		PLUGIN:SetBodyGroupsFromTable(self, class.bodyGroupsListq)
	end;
	...
	-- Set 'permanent' personal bodygroupsq, overwrite previous bodygroups
	if (bodyGroupsListq) then
		PLUGIN:SetBodyGroupsFromTable(self, bodyGroupsListq)
	end;
