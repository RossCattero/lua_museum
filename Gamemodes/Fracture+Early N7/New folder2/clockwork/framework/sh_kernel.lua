...
if (SERVER) then
	...
	local function UpdateCenterHint(index, hintInfo, iCount)
		...
		local idealY = (ScrH() * 0.9) + (height * (index - 1));
	...
	function Clockwork.kernel:DrawDateTime()
		...
		if (Clockwork.LastDateTimeInfo and Clockwork.LastDateTimeInfo.y > info.y) then
			...
			draw.RoundedBox( 2, x, y + 8, width, height, Color( 0, 0, 0, 150 ) );
			y = y + height + 16;
			...
			if (self:CanCreateInfoMenuPanel() and self:IsInfoMenuOpen()) then
				...
				draw.RoundedBox( 4, Clockwork.InfoMenuPanel.x - 4, Clockwork.InfoMenuPanel.y - 4, Clockwork.InfoMenuPanel:GetWide() + 8, Clockwork.InfoMenuPanel:GetTall() + 8, Color( 0, 0, 0, 150 ) );
		...
		if (texInfo.shouldDisplay) then
			...
			for k, v in pairs(Clockwork.limb.hitGroups) do
				...
				local name = Clockwork.limb:GetName(k);
				local Ttext = 'nil'
				local htemp = Clockwork.Client:GetSharedVar('headtemp')
				local btemp = Clockwork.Client:GetSharedVar('bodytemp')
				local ltemp = Clockwork.Client:GetSharedVar('legstemp')
				local hatemp = Clockwork.Client:GetSharedVar('handstemp')

				surface.SetDrawColor(limbColor.r, limbColor.g, limbColor.b, 150);
				surface.SetMaterial(texInfo.textures[k]);
				surface.DrawTexturedRect(x, y, width, height);
				
				if name == 'LimbRightArm' || name == 'LimbLeftArm' then
					if hatemp then
						if hatemp >= 45 && hatemp <= 55 then
							Ttext = 'Нормальная температура'
						elseif hatemp < 45 then
							Ttext = 'Замерзаете'
						elseif hatemp > 55 then
							Ttext = 'Жарко'
						end;
					end;
				elseif name == 'LimbRightLeg' || name == 'LimbLeftLeg' then
					if ltemp then
						if ltemp >= 45 && ltemp <= 55 then
							Ttext = 'Нормальная температура'
						elseif ltemp < 45 then
							Ttext = 'Замерзаете'
						elseif ltemp > 55 then
							Ttext = 'Жарко'
						end;
					end;
				elseif name == 'LimbStomach' || name == 'LimbChest' then
					if btemp then
						if btemp >= 45 && btemp <= 55 then
							Ttext = 'Нормальная температура'
						elseif btemp < 45 then
							Ttext = 'Замерзаете'
						elseif btemp > 55 then
							Ttext = 'Жарко'
						end;
					end;
				elseif name == 'LimbHead' then
					if htemp then
						if htemp >= 45 && htemp <= 55 then
							Ttext = 'Нормальная температура'
						elseif htemp < 45 then
							Ttext = 'Замерзаете'
						elseif htemp > 55 then
							Ttext = 'Жарко'
						end;
					end;
				end;

				if Clockwork.attributes:Get("Восприятие") < 6 then
					if limbHealth == 100 then
						limbHealthText = "В порядке | "..Ttext
					elseif limbHealth >= 75 then
						limbHealthText = "Болит | "..Ttext
					elseif limbHealth >= 50 then
						limbHealthText = "Ранение | "..Ttext
					elseif limbHealth >= 25 then
						limbHealthText = "Тяжелое ранение | "..Ttext
					elseif limbHealth >= 0 then
						limbHealthText = "Сломано | "..Ttext
					end;
				elseif Clockwork.attributes:Get("Восприятие") >= 6 then
					limbHealthText = limbHealth.."% | "..Ttext
				end;

				limbInfo[newIndex] = {
					color = limbColor,
					text = L("LimbStatus", L(texInfo.names[k]), limbHealthText)
				};
	...
	function Clockwork.kernel:DrawPlayerInfo(info)
		...
		if (!Clockwork.plugin:Call("PreDrawPlayerInfo", boxInfo, information, subInformation)) then
			...
			if (#information > 0 and boxInfo.drawBackground) then
				...
				draw.RoundedBox(boxInfo.cornerSize, x, y, width, height - ((textHeight + 12) * #subInformation), Color(0, 0, 0, 150))
			end;
	...
	function Clockwork.kernel:DrawPlayerInfoSubBox(text, x, y, width, boxInfo)
		...
		if (boxInfo.drawBackground) then
			...
			draw.RoundedBox(4,x,y,width,boxHeight,Color(0, 0, 0, 150))
		end;
	...
	function Clockwork.kernel:HandleItemSpawnIconClick(itemTable, spawnIcon, Callback)
		...
		if itemTable:IsBasedFrom("food_base") && itemTable:GetData("useAmount") > 0 then
			if (itemTable.OnUse) then
				itemFunctions[#itemFunctions + 1] = itemTable("useText", "Use");
			end;
		elseif !itemTable:IsBasedFrom("food_base") then
			if (itemTable.OnUse) then
				itemFunctions[#itemFunctions + 1] = itemTable("useText", "Use");
			end;
		end;
	...
	function Clockwork.kernel:DrawMarkupToolTip(markupObject, x, y, alpha)
		...
		draw.RoundedBox( 2, x - 8, y - 8, width + 16, height + 16, Color( 0, 0, 0, alpha ) );
	...
	function Clockwork.kernel:CreateCustomCategoryPanel(categoryName, parent)
		...
		collapsibleCategory.Paint = function( self, x, y )
			if not collapsibleCategory then return end
			draw.RoundedBox(4, 0, 0, x, y, Color(0, 0, 0, 150))
		end;		

		table.insert(parent.CategoryList, collapsibleCategory )
	...
	function Clockwork.kernel:CalculateScreenFading()
		if (Clockwork.plugin:Call("ShouldPlayerScreenFadeBlack")) then
			...
			draw.RoundedBox(0, 0, 0, ScrW(), ScrH(), Color(0, 0, 0, Clockwork.BlackFadeIn))
		else
			if (Clockwork.BlackFadeIn) then
			...
			if (Clockwork.BlackFadeOut) then
				...
				draw.RoundedBox(0, 0, 0, ScrW(), ScrH(), Color(0, 0, 0, Clockwork.BlackFadeOut))
		...
		if (cinematicInfo) then
			...
			if (cinematicInfo.title) then
				...
				if (cinematicInfo.text) then
					...
					-- );
					draw.RoundedBox( 2, 0, textPosY - 80, screenWidth, textHeight + smallTextHeight + 160, Color(0, 0, 0, boxAlpha) );
				else
					-- self:DrawGradient(
		...
		if (!doorData.hitWorld) then
			...
			if (name or text) then
				...
				cam.Start3D2D(doorData.position, doorData.angles, nameScale);
					draw.RoundedBox( 2, -(longWidth / 2) - 128, frontY - 8, longWidth + 256, longHeight, Color(100, 100, 100, boxAlpha) );
