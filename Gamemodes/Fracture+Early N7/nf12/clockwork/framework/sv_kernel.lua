...
--[[
	...
	@returns {Unknown}
--]]
function Clockwork:SetupMove(player, moveData) end;
...
function Clockwork:PlayerCanDeleteCharacter(player, character)
	if (cwConfig:Get("cash_enabled"):Get() and character.cash < cwConfig:Get("default_cash"):Get()) then
		if (!character.data["CharBanned"]) then
			return "Вы не можете удалить персонажа, у которого денег меньше, чем "..cwKernel:FormatCash(cwConfig:Get("default_cash"):Get(), nil, true)..".";
		end;
	end;
end;
...
function Clockwork:PlayerCanSwitchCharacter(player, character)
	if (!player:Alive() and !player:IsCharacterMenuReset() and !player:GetSharedVar("CharBanned")) then
		return "Вы не можете менять персонажей сейчас!";
...
function Clockwork:PlayerCanUseCharacter(player, character)
	if (character.data["CharBanned"]) then
		return character.name.." забанен и не может быть использован!";
	end;
...
function Clockwork:EntityHandleMenuOption(player, entity, option, arguments)
	...
	local antidupe = 1 + math.random(0.0000, 0.0050);
...
function Clockwork:PlayerLoadout(player)
	...
	if (cwConfig:Get("give_hands"):Get() or !player:GetFaction() == FACTION_CREMATOR) then
		cwPly:GiveSpawnWeapon(player, "cw_hands");
	end;

	if (cwConfig:Get("give_keys"):Get() or !player:GetFaction() == FACTION_CREMATOR) then
			...
			if (damageInfo:IsDamageType(DMG_CRUSH) and damageInfo:GetDamage() < 10) then
				...
				if (damageInfo:GetDamage() > 0) then
					...
					if (cwPlugin:Call("PlayerCanTakeKnockbackFromDamage", entity, damageInfo)) then
						player:SetVelocity(cwKernel:ConvertForce(damageInfo:GetDamageForce() * 10, 200));
					end;
