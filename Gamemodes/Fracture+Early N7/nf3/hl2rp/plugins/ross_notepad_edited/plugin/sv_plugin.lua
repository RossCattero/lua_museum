...
cable.receive('EditPadSettings', function(player, type, value, tbl)
    ...
    if type == "Owner" then
        if ent:GetClass() == "cw_item" && player:GetPos():Distance(ent:GetPos()) < 90 && git(ent, "baseItem") == "ross_notepad_base" then
        	if (git(ent, "data")["NotepadTableForMe"]["owner"] != "") and (git(ent, "data")["NotepadTableForMe"]["owner"] != nil) then
        		Clockwork.player:Notify(player, "Уже есть Владелец этого блокнота!")
        		return 
        	end
            ...
            local notepad = player:FindItemByID(tbl["additionalInfo"]["uniqueID"], tbl["additionalInfo"]["ItemID"]);
            if notepad then
            	if (notepad:GetData("NotepadTableForMe")["owner"] != "") and (notepad:GetData("NotepadTableForMe")["owner"] != nil) then
            		Clockwork.player:Notify(player, "Уже есть Владелец этого блокнота!")
            		return 
            	end
    ...
    if type == "Pick" then
        if ent:GetClass() == "cw_item" && player:GetPos():Distance(ent:GetPos()) < 90 && git(ent, "baseItem") == "ross_notepad_base" then
        	if (git(ent, "data")["NotepadTableForMe"]["owner"] != player:Name()) then
        		Clockwork.player:Notify(player, "Вы не Владелец этого блокнота!")
        		return
        	end
            ...
            local notepad = player:FindItemByID(tbl["additionalInfo"]["uniqueID"], tbl["additionalInfo"]["ItemID"]);
            if notepad then
            	if (notepad:GetData("NotepadTableForMe")["owner"] != player:Name()) then
            		Clockwork.player:Notify(player, "Вы не Владелец этого блокнота!")
        			return
        		end
        		---------------------------------------------
