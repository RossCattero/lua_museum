...
function cwStamina:PlayerPunchThrown(player)
	local gasmask = player:GetCharacterData("GasMaskInfo") || 0
	local decrease = 5 / (gasmask + 5);
...
function cwStamina:PlayerThink(player, curTime, infoTable)
	local maxStamina = player:GetCharacterData("MaxStamina") - ( 50 - (player:GetNeed("thirst") * 0.5) )
	local regenScale = 0.2
	local drainScale = 0.4
	...
	local decrease = (drainScale + (drainScale - healthScale)) - ((drainScale * 0.5) - (100/chestDamage*0.5) + (tonumber(number) * 0.15) );
	...
	infoTable.runSpeed = math.Clamp(newRunSpeed - (diffRunSpeed - ((diffRunSpeed / 100) * player:GetCharacterData("Stamina"))), 10, maxRunSpeed);

	if stamina <= 2 then
		player:ConCommand('-speed');
	end;
