    ...
    if !player.walking || curTime >= player.walking then
        ...
        if inc then
            ...
            elseif running then
                ...
                player:SetHeadTemp( mc(player:GetHeadTemp() + 0.1, 0, 100 ) )
                player:SetBodyTemp( mc(player:GetBodyTemp() + 0.1, 0, 100 ) )
                player:SetLegsTemp( mc(player:GetLegsTemp() + 0.1, 0, 100 ) )
                player:SetHandsTemp( mc(player:GetHandsTemp() + 0.1, 0, 100 ) )
            end;
        end;
...
cable.receive('StartSleeping', function(player, num)
    ...
    if Schema:PlayerIsCombine(player) && !player:IsRagdolled() then
        Clockwork.chatBox:SendColored(player, Color(255, 100, 100), "Вам запрещено использовать команду сна!")
        return;
    end;
    if (Schema:PlayerIsCombine(player) && !player:IsRagdolled()) || (player:GetForcedAnimation() == nil || !table.HasValue(animationTable, player:GetForcedAnimation()['animation'])) then
