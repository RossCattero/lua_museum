...
function PANEL:ResetTextAndPositions()
	...
	if (schemaLogo == "") then
		...
		self.titleLabel:SetPos((scrW * 0.15) - (self.titleLabel:GetWide() / 2), scrH * 0.4);
		self.subLabel:SetPos((scrW * 0.15) - (self.subLabel:GetWide() / 2), self.titleLabel.y + self.titleLabel:GetTall() + 8);
	end;
...
function PANEL:SetModelPanelModel(model)
	...
	local modelClass = Clockwork.animation:GetModelClass(modelPanel.Entity:GetModel());
	...
	-- if (sequence) then
		if (modelClass == "maleHuman" || modelClass == "femaleHuman" || modelClass == "civilProtectionfemale") then
			modelPanel.Entity:ResetSequence(4);
		else
			modelPanel.Entity:ResetSequence(7);
		end;
	-- end;
end;
...
function PANEL:OnNext()
	...
	if (!Clockwork.faction.stored[self.info.faction].GetName) then
		if (IsValid(self.fullNameTextEntry)) then
			...
			local forefind = select(3, string.find(self.info.forename, "([абвгдеёжзийклмнопрстуфхцчшщъьыэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЬЫЭЮЯ%a]+)")) local surfind = select(3, string.find(self.info.surname, "([абвгдеёжзийклмнопрстуфхцчшщъьыэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЬЫЭЮЯ%a]+)"))
			...
			if (forefind != self.info.forename or surfind != self.info.surname) then
				Clockwork.character:SetFault("Ваши имя и фамилия не должны содержать знаки пунктуации, пробелы или цифры!");
				return false;
			end;
