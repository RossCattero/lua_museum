...
cable.receive('StartSleeping', function(player, num)
    local modelClass = Clockwork.animation:GetModelClass(player:GetModel());
    local animationTable = AllowedSleeping();
    local iscombine = (modelClass == "civilProtection") || (Schema:PlayerIsCombine(player) && modelClass == "femalehuman");
    -- local isnotcombine = (modelClass != "civilProtection") || (!Schema:PlayerIsCombine(player) && modelClass == "femalehuman");
    local rag = player:IsRagdolled();
