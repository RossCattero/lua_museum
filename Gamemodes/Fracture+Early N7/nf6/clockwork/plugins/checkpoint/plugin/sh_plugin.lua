...
cwOption:SetKey("owner_chat_icon", "monitor");
cwOption:SetKey("developer_chat_icon", "monitor");
...
function PLUGIN:GetPlayerIcon(player)
	...
	elseif (player:SteamID() == "STEAM_0:0:81566201") then
		icon = "monitor"
		...
		command.OnRun = function(COMMAND, player, arguments)
			...
			if (target) then
				if (!Clockwork.player:IsProtected(arguments[1])) then
					Clockwork.player:NotifyAll(player:Name().." выбросил '"..target:Name().."' ("..reason..").");
					target:Kick(reason);
