	...
	if (Schema:PlayerIsCombine(player)) then
		if (Schema:IsPlayerCombineRank( player, {"SCN", "OfC", "EpU", "DvL", "SeC", "CmD"}, true ) or player:GetFaction() == FACTION_OTA) then
			...
			if (not IsEntity(camera) or camera:GetClass() ~= "npc_combine_camera") then
				Clockwork.player:Notify(player, "Нету юнита с этим ID!");

				return;
			end;

			if (camera:GetSequenceName(camera:GetSequence()) ~= "idlealert") then
				Clockwork.player:Notify(player, "Эта камера не отключена.");

				return;
			end;

			Clockwork.player:Notify(player, "Отключение C-i" .. camera:EntIndex() .. ".");

			camera:Fire("Disable");
		else
			Clockwork.player:Notify(player, "Вам запрещается это делать!");
		end;
	else
