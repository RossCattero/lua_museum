...
cwCTO.socioStatus = cwCTO.socioStatus or "ЗЕЛЕНЫЙ";
...
function cwCTO:HUDPaintForeground()
	...
	if (Schema:PlayerIsCombine(Clockwork.Client) && Clockwork.Client:GetSharedVar("GasMaskInfo") >= 2) then
			...
			elseif (not (not data.isLost and Clockwork.player:IsNoClipping(unit))) then
				...
				if (toScreen.visible) then
					...
					if (data.isLost) then
						...
						toScreen.y = toScreen.y + fontHeight;
						draw.SimpleText("<:: Удалется " .. timeUntil .. "s ::>", "BudgetLabel", toScreen.x, toScreen.y, colorRed, 1, 1);
					else
						local text2 = "<:: Принимается " .. timeSince .. "s ::>";
						draw.SimpleText(text, "BudgetLabel", toScreen.x, toScreen.y, color, 1, 1);
						toScreen.y = toScreen.y + fontHeight;
		...
		for i, data in ipairs(self.requestLocations) do
			if (curTime - data.time >= requestExpiry) then
				...
				if (toScreen.visible) then
					...
					draw.SimpleText("<:: Запрос поддержки ::>", "BudgetLabel", toScreen.x, toScreen.y, requestColor, 1, 1);
					toScreen.y = toScreen.y + fontHeight;
					draw.SimpleText(showDetail and text2 or lowDetailText, "BudgetLabel", toScreen.x, toScreen.y, colorWhite, 1, 1);
					toScreen.y = toScreen.y + fontHeight;
					draw.SimpleText("<:: Удалется " .. timeUntil .. "s ::>", "BudgetLabel", toScreen.x, toScreen.y, colorRed, 1, 1);
		...
		for combineCamera, data in pairs(self.cameraData) do
			if (IsValid(combineCamera)) then
					...
					for player, vios in pairs(data) do
						for i, vio in ipairs(vios) do
							if (vio == self.VIOLATION_RUNNING) then
								violations[#violations + 1] = "<:: 1xБег ::>";
							elseif (vio == self.VIOLATION_JOGGING) then
								violations[#violations + 1] = "<:: 1xБег трусцой ::>";
							elseif (vio == self.VIOLATION_JUMPING) then
								violations[#violations + 1] = "<:: 1xПрыжок ::>";
							elseif (vio == self.VIOLATION_CROUCHING) then
								violations[#violations + 1] = "<:: 1xПрисяд ::>";
							elseif (vio == self.VIOLATION_FALLEN_OVER) then
								violations[#violations + 1] = "<:: 1xЛежит ::>";
							end;
						end;
					end;
				end;
				...
				if (toScreen.visible) then
					...
					if (type(data) == "table") then
						local text2 = "<:: " .. table.Count(data) .. " в пределах видимости ::>";
						...
						if (#violations > 0) then
							toScreen.y = toScreen.y + fontHeight;
							draw.SimpleText("<:: Нарушения в пределах видимости ::>", "BudgetLabel", toScreen.x, toScreen.y, colorRed, 1, 1);
		...
		for _, v in pairs( _player.GetAll() ) do
			if (not (Schema:PlayerIsCombine(v) and not v:GetCharacterData("IsBiosignalGone") && Clockwork.Client:GetSharedVar("GasMaskInfo") >= 2) and
			beholderEyePos:Distance(v:GetPos()) <= maximumDistance and not Clockwork.player:IsNoClipping(v)) then
				...
				if (toScreen.visible and beholder:IsLineOfSightClear(v)) then
					...
					v:GetCharacterData("IsCIDTagGone") and CID ~= "" && !v:GetFaction() == FACTION_MPF) then
						local text = "<:: c#" .. CID .. " ::>";
					...
					if (v:IsRunning() && !v:GetFaction() == FACTION_MPF) then violations[#violations + 1] = "<:: 1xБег ::>"; end;
					if (v:IsJogging() && !v:GetFaction() == FACTION_MPF) then violations[#violations + 1] = "<:: 1xБег трусцой ::>"; end;
					if (v.m_bJumping && !v:GetFaction() == FACTION_MPF) then violations[#violations + 1] = "<:: 1xПрыжок ::>"; end;
					if (v:Crouching() && !v:GetFaction() == FACTION_MPF) then violations[#violations + 1] = "<:: 1xПрисяд ::>"; end;
					if (v:GetRagdollState() ~= RAGDOLL_NONE and v:GetRagdollState() ~= RAGDOLL_RESET) then
						violations[#violations + 1] = "<:: 1xЛежит ::>";
					end;

					if (#violations > 0 && !v:GetFaction() == FACTION_MPF) then
						draw.SimpleText("<:: Возможно нарушение ::>", "BudgetLabel", toScreen.x, toScreen.y, colorRed, 1, 1);
...
function cwCTO:HUDPaintTopScreen()
	...
	if (Schema:PlayerIsCombine(Clockwork.Client) && Clockwork.Client:GetSharedVar("GasMaskInfo") > 0) then
		local height = draw.GetFontHeight("BudgetLabel");

		local socioColor = self.sociostatusColors[self.socioStatus] or colorWhite;

		if (self.socioStatus == "ЧЕРНЫЙ") then
		...
		draw.SimpleText("<:: СОЦИОСТАТУС = "..self.socioStatus.." ::>", "BudgetLabel", info.x, info.y, socioColor, TEXT_ALIGN_RIGHT);
		draw.SimpleText("<:: ВАШ СТАТУС = "..Clockwork.Client:GetSharedVar("status").." ::>", "BudgetLabel", info.x, info.y + 15, Color(20, 255, 20, 255), TEXT_ALIGN_RIGHT);
...
function Schema:AddCombineDisplayLine(text, color)
	if (self:PlayerIsCombine(Clockwork.Client) && Clockwork.Client:GetSharedVar("GasMaskInfo") > 0) then
		if (not self.combineDisplayLines) then
			self.combineDisplayLines = {};
		end;

		if (color or !Clockwork.Client:GetCharacterData("IsBiosignalGone")) then
...
function cwCTO:HUDPaintEntityTargetID(entity, info)
	...
	if (entity:GetClass() == "cw_ctocameraterminal") then
		info.y = Clockwork.kernel:DrawInfo("Терминал камер", info.x, info.y, colorTargetID, info.alpha);
		info.y = Clockwork.kernel:DrawInfo("Используется для просмотра терминалов.", info.x, info.y, colorWhite, info.alpha);
	end;
end;
...
function cwCTO:GetEntityMenuOptions(entity, options)
	if (entity:GetClass() == "cw_ctocameraterminal") then
		options["Отключить"] = "cwCameraOpen0";
		for k, v in pairs( ents.FindByClass("npc_combine_camera") ) do
			options["Посмотреть C-i" .. v:EntIndex()] = "cwCameraOpen" .. v:EntIndex();
		end;
	end;
end;
