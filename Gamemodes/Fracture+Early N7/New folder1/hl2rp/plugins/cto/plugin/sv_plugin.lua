					...
					elseif (#data[player] < 1) then
						...
						if (#violations > 0) then
							if not (Schema:PlayerIsCombine(player) and not player:GetCharacterData("IsBiosignalGone")) then
								...
								Schema:AddCombineDisplayLine("Нарушение передвижения замечены C-i" .. combineCamera:EntIndex() .. "...", Color(255, 128, 0, 255));
							end;
						end;
					end;
				end;
...
function cwCTO:DoPostBiosignalLoss(player)
	...
	Schema:AddCombineDisplayLine("Загрузка утерянного биосигнала...", Color(255, 255, 255, 255));
	Schema:AddCombineDisplayLine("ВНИМАНИЕ! Потерян биосигнал юнита: "..digits..", в локации: "..location.."...", Color(255, 0, 0, 255));
	for k, v in ipairs( _player.GetAll() ) do
		...
		elseif (not bEnable and isDisabledAlready) then
			...
			if (bEnable) then
				...
				timer.Simple(0.1, function()
					...
					-- Alert this unit.
					Schema:AddCombineDisplayLine("Соединение обновлено...", Color(0, 255, 0, 255), player);

					local digits = string.match(player:Name(), "%d%d%d%d?%d?") or 0;

					-- Alert all units.
					Schema:AddCombineDisplayLine("Найден биосигнал...", Color(255, 255, 255, 255));
					Schema:AddCombineDisplayLine("ВНИМАНИЕ! Несоответствующий биосигнал юнита: "..digits.." обнаружен в "..location.."...", Color(0, 255, 0, 255));
				...
				-- Alert this unit.
				Schema:AddCombineDisplayLine("ОШИБКА! Выключение...", Color(255, 0, 0, 255), player);
...
function cwCTO:PostPlayerSpawn(player, lightSpawn, changeClass, firstSpawn)
	...
	if (Schema:PlayerIsCombine(player)) then
		Clockwork.datastream:Start(player, "RecalculateHUDObjectives", {cwCTO.socioStatus});

		if (player:GetCharacterData("IsBiosignalGone")) then
			Clockwork.player:Notify(player, "У вас нету биосигнала.");
		end;
	end;
...
function cwCTO:DispatchRequestSignal(player, text)
	...
	Schema:AddCombineDisplayLine("Найден запрос о помощи...", Color(175, 125, 100, 255));
end;
...
function cwCTO:PlayerRagdolled(player, state, ragdoll)
	if (Schema:PlayerIsCombine(player) and not player:GetCharacterData("IsBiosignalGone")) then
		if (state == RAGDOLL_KNOCKEDOUT) then
			...
			Schema:AddCombineDisplayLine("Загрузка травм...", Color(255, 255, 255, 255));
			Schema:AddCombineDisplayLine("ВНИМАНИЕ! Юнит "..digits.." потерял сознание в "..location.."...", Color(255, 0, 0, 255));
		end;
	end;
end;
...
function cwCTO:Initialize()
	...
	function Schema:AddCombineDisplayLine(text, color, player, exclude)
		...
		if (string.StartWith(text, "ВНИМАНИЕ! Юнит получает урон в ") and player == nil and IsValid(exclude)) then
			local location = self:PlayerGetLocation(exclude);
			local digits = string.match(exclude:Name(), "%d%d%d%d?%d?") or 0;

			text = "ВНИМАНИЕ! Юнит "..digits.." получает урон в "..location.."...";
		end;

		if (string.StartWith(text, "Загружены последние задачи...")) then
			timer.Simple(0.1, function()
				local players = {};
