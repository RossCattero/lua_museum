...
function ENT:StartTouch( ent )
	if ent:GetClass() == "cw_item" && git(ent, "uniqueID") == "citizen_civ_card" && self:GetTimeLeft() == 0 then 
		self:SetTimeLeft(76)
		Schema:AddCombineDisplayLine("Монитор: #"..self:GetNumberDone()..". Запрос помощи!", Color(255, 255, 255, 255), nil, player);
	end;
end;
...
function ENT:OnTakeDamage( dmginfo )
	...
	if self:Health() == 0 && self.boolingan == false then 
		...
		Schema:AddCombineDisplayLine("Монитор: #"..self:GetNumberDone()..". КРИТИЧЕСКИЙ УРОН, ПОЛОМКА!", Color(255, 255, 255, 255), nil, player);
			local effect = EffectData()
