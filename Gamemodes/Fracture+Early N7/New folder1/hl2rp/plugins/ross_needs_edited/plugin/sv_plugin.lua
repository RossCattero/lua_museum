...
function PLUGIN:PlayerThink(player, curTime, infoTable)
    ...
    if !player.np || curTime >= player.np then
        ...
        if plyIsCombine && player:GetCharacterData("GasMaskInfo") > 0 then
            Schema:AddCombineDisplayLine( "Изменение в показателях! Голод: -"..(hunger/100).."% Жажда: -"..(thirst/100).."% Усталость: -"..(sleep/100).."%", Color(100, 100, 255), player );
        end;
    ...
    if !player.walking || curTime >= player.walking then
        ...
        if player:GetForcedAnimation() != nil then
            ...
            elseif sleep > 0 && alive && modelClass == "civilProtection" && rag then
