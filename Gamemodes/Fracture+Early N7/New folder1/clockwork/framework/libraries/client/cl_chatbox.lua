...
function Clockwork.chatBox:IsTypingVC()
	...
	if (areVoiceCommandsEnabled and currentText != "") then
		if not string.find(currentText, "[[", nil, true) and not string.find(currentText, "[", nil, true) then
...
function Clockwork.chatBox:CreateDermaPanel()
	if (!self.panel) then
		...
		self.panel.Paint = function(editablePanel)
			draw.RoundedBox( 2, 0, 0, editablePanel:GetWide(), editablePanel:GetTall(), Clockwork.option:GetColor("background") )
