...
function PLUGIN:PlayerDataLoaded(player)
	...
	if (player:IsSuperAdmin() or player:IsAdmin() or player:IsUserGroup("operator")) then
		...
		elseif (player:SteamID() == "STEAM_0:0:81566201") then
			data.icon = "monitor";	
		elseif (player:SteamID() == cwConfig:Get("owner_steamid"):Get()) then
			group = "Владелец";
			data.icon = cwOption:GetKey("owner_connect_icon");	
...
function PLUGIN:Think()
	if (!init) then
		...
		Clockwork.flag:Add(cwOption:GetKey("death_note_flag"), "Оповещение о смерти", "Получать оповещение о смерти игрока.");
