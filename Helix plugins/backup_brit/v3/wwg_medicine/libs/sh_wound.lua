...
/*
	The number of the last wound ID;
	@table ix.wound.index
	@realm [shared]
	@usage print(ix.wound.index)
	> 0;
*/
ix.wound.index = ix.wound.index || 0;
...
function ix.wound.New( uniqueID, id )
	...
	ix.wound.index = ix.wound.index + 1;
	id = id || ix.wound.index;
...
function ix.wound.Instance( uniqueID, characterID, id, bone, time, data )
	...
	body:BoneInsert(bone, wound:GetID())
...
function ix.wound.CreateBleed( client, bone )
	...
	hook.Run("CreateWound", client, wound:GetID())
