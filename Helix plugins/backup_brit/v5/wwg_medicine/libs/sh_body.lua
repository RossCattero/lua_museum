...
/*
	Check if entity can receive a wound;
	@realm [shared]
	@argument [entity] [Entity object] The entity to check.
	@usage print(ix.body.CanReceiveWounds(NULL))
	> false
*/
function ix.body.CanReceiveWounds(entity)
	return entity && entity:IsValid() && !entity:GetNoDraw() && (entity:IsPlayer() || entity:GetNetVar("player"))
end;
	...
	function ix.body.Remove( characterID, erase )
		if erase then
			for id, wound in pairs( ix.body.characters[ characterID ] ) do
				ix.wound.instances[ wound ]:Remove();
			end
