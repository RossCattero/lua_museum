...
net.Receive("NETWORK_DISEASE", function(len)
	local id = net.ReadUInt(16);
	local uniqueID = net.ReadString();

	local disease = ix.disease.New( uniqueID, id );
	disease.charID = LocalPlayer():GetCharacter():GetID();
	disease.occured = os.time();
end)

net.Receive("NETWORK_DISEASE_REMOVE", function(len)
	local id = net.ReadUInt(16);

	if ix.disease.instances[ id ] then
		ix.disease.instances[ id ] = nil;
	end
end)