...
/*
	...
	@usage ix.body.instances[1]:WoundAdd("torso", 1)
*/

function BODY:WoundAdd(bone, id)
...
/*
	Remove wound from bone;
	@realm [shared]
	@argument [bone] [string] The string limb name.
	@argument [id] [number] The number id of wound.
	@usage ix.body.instances[1]:WoundRemove("torso", 1)
*/

function BODY:WoundRemove(bone, id)
	local wound = ix.wound.instances[ id ];
	if !wound then return end;
...
if SERVER then
	// TODO: doc
	function BODY:SetStats( walkSpeed, runSpeed, crouchSpeed )
		local client = self:GetOwner();

		if client then
			client:SetRunSpeed( client:GetRunSpeed() * ( 1 + (runSpeed/100) ) )
			client:SetCrouchedWalkSpeed( client:GetCrouchedWalkSpeed() * ( 1 + (crouchSpeed/100) ) )
			client:SetSlowWalkSpeed( client:GetSlowWalkSpeed() * (1 + (walkSpeed/100)) )
			client:SetWalkSpeed( client:GetWalkSpeed() * (1 + (walkSpeed/100)) )
		end
	end;
end
