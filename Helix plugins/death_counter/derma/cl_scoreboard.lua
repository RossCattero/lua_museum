-- I've taken a default scoreboard derma because there's no opporunity to get scoreboard and edit it easily without editing a schema itself.
...
function PANEL:Update()
	...
	if (self.name:GetText() != name) then
		...
		local death = client:GetLocalVar('deathCounter')
		print(EnableDeathCounter)
		if EnableDeathCounter then
			self.name:SetText(self.name:GetText() .. " ["..death.."]")
		end;
	end
