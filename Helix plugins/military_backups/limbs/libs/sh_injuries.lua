...
-- Сколько времени потребуется, чтобы исправиться или загноиться. Если не установлено, то будет использована константа "LIMB.ROT_TIME"
injTime = 60,
...
stageCritical = 0, // Критическая стадия раны, которая сама по себе не проходит
...
function LIMB:CanRot(index)
	return self.INJURIES[index] && self.INJURIES[index].canInfect
end;