...
local PANEL = {}
local CharLoadGrad = Material( "gui/gradient" )
...
function PANEL:Init()
	...
	local PanelChars = self.panel:Add("Panel")
	PanelChars:SetSize(halfWidth, 800)
	PanelChars.Paint = function( s, w, h )
		draw.RoundedBox( 0, 0, 0, w, h, Color(0, 0, 0, 100) )
	end;

	self.characterList = PanelChars:Add("DScrollPanel")
...
function PANEL:SetDeletePanel(onclick)
...
function PANEL:Populate(ignoreID)
	...
	for i = 1, #ix.characters do
		...
		local btnChar = self.characterList:Add( "ixMenuSelectionButton" )
		btnChar:SetText( '' )
		btnChar.character = character
		btnChar:SetHeight(60)
		btnChar:SetButtonList(self.characterList.buttons)
		btnChar:Dock( TOP )
		btnChar:DockMargin( 5, 5, 5, 5 )
		function btnChar:Paint(w, h)
			surface.SetDrawColor( color.r, color.g, color.b, color.a )
			surface.SetMaterial( CharLoadGrad );
			surface.DrawTexturedRect( 0, 0, w, h );

			draw.DrawText( character:GetName().." / "..ix.faction.Get(index).name, "ixMenuButtonFont", 10, 10, Color(255, 255, 255), TEXT_ALIGN_LEFT )
		end;		
		btnChar.OnSelected = function(panel)
			self:OnCharacterButtonSelected(panel)
		end
		btnChar.DoRightClick = function()
			self:SetDeletePanel(true)
		end;
		...
		if (localCharacter and character:GetID() == localCharacter:GetID()) then
			btnChar:SetSelected(true)
			self.characterList:ScrollToChild(btnChar)
