...
local buttonList = {
		...
		},
		{
			...
			click = function(btn, data)
				local info = PLUGIN.bankingAccounts[data];
				...
				function(text) 
						...
						sound.Play( PLUGIN.PenWrite, LocalPlayer():GetPos(), 50 )						
		...
		{
			index = "itemsLook",
			name = "Look at banking inventory",
			click = function(btn, data)
				sound.Play( PLUGIN.PenWrite, LocalPlayer():GetPos(), 50 )
			end,
		},
...
function PANEL:ReloadItems()	
		...
		while (i <= max) do
			...
			local id = HASH_MASSIVE[i];
				...
				local account = PLUGIN.bankingAccounts[id] 
				if account then
						...
						while (j <= #PLUGIN.enums) do
								...
								if info then
										...
										local value = info.tomoney && nut.currency.get(account[info.id]) || account[info.id];
		...
		self:ButtonsCheck();
		surface.PlaySound(PLUGIN.BookFlip)
end;
...
function PANEL:Paint( w, h )
		...
		surface.SetMaterial( PLUGIN.BankBook )
...
function PANEL:Populate(i)
	...
	if i == 1 then
			...
			self.funds:SetText("Funds: " .. nut.currency.get(PLUGIN.moneyFunds));
