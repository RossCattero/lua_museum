...
function user:ReceiveBankingData()
		if self:getChar():hasFlags(BANKING_BOOK_FLAG) then
			netstream.Start(self, 'Banking::SyncBanking', PLUGIN.bankingAccounts, PLUGIN.moneyFunds)
		end;
end;

function user:HasBankingAccount()
		local char = self:getChar();
		local hash = char:getData("Hash");

		return PLUGIN.bankingAccounts[hash]
end;

function user:HasLoan()
		local bank = self:HasBankingAccount();

		return bank && bank.loan > 0
end;
...
function user:CanIncreaseDeposit()
		...
		return bank && statuses[bank.status] > 0;
end;