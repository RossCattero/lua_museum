...
nut.command.add("charsearch", {
	onRun = function(client, arguments)
		...
		if !client:IsTargetTurned(target) then
				client:notify("Target should be turned back to you to do that.")
				return;
		end

		if (IsValid(target) and target:IsPlayer() && client:IsTargetTurned(target) and target:getNetVar("restricted")) then
