...
function PLUGIN:canCreate(ply, recipee)
	...
	// Ross: check if result fits in inventory;
	if recipe.result then
		if type(result) == "string" then
				local item = nut.item.list[result]
				return item && ply:getChar():getInv():canItemFitInInventory(item, item.width, item.height)
		elseif type(result) == "table" then
				for k, v in pairs(result) do				
					local item = nut.item.list[result]
					if !(item && ply:getChar():getInv():canItemFitInInventory(item, item.width, item.height)) then
						ply:notify("The item can't fit in your inventory!")
						return false
					end
				end
		end
	...
	if(recipe.requirements) then
			if ( char ) then
				local items, req = {}, recipe.requirements
	...
	if(recipe.handpick) then
		if(char) then
			local item, notif = recipe.handpick(char:getInv():getItems())
			if(item) then
				return true, item
			else
