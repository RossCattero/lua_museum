...
netstream.Hook('jobVendor::pickWork', function(client, workName)
		...
		if PLUGIN.jobsList[workName] then
				if !client:CanPickJob(workName) then
				...
				timer.Create(uniqueID, 1, PLUGIN.jobsList[workName].timeForRepair, function()
						...
						if client:IsValid() && timer.RepsLeft(uniqueID) <= 0 then
								client:ClearJob()
						end;
				end);
		end;
...
// Переделать систему назначения идентификаторов. Глупо было делать их в одном массиве по разным номерам. Теперь хуй достучишься
