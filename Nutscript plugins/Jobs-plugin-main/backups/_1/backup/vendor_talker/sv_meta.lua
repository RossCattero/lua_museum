...
function user:CreateJob(data)
		...
		local job = JOB:new(index, {price = data.price, reputation = data.rep, entity = ent})
...
function user:ClearJob()
		...
		local ent = PLUGIN.getJobs[job].entity;
...
function user:CallThePositionsAmount()
		...
		for k, v in pairs(jobPoses) do
			local i = 0;
			while (i <= #jobPoses[k]) do
						if !positions[k] then
								positions[k] = 0;
						end
					if jobPoses[k][i] && !jobPoses[k][i][4] then
