...
function PANEL:Init()
    ...
    self.addJob.DoClick = function(btn)
				if self.jobType.selected && !btn.disabled then
						...
						ADDJOB_SETTINGS.lastJob = self.jobType.selected
		...
		for k, v in pairs(PLUGIN.jobsList) do
				index = index + 1;
				self.jobType:AddChoice(v.title, k, ADDJOB_SETTINGS.lastJob == index)
				if ADDJOB_SETTINGS.lastJob == index then
						self.addJob.disabled = false;
				end
		end
end
...
function PANEL:Refresh(data)
		...
		local i = 1;
		while (i <= #data) do
				local line = self.remList:AddLine(data[i].name, data[i].position, data[i].ply)
				line.jobID = i;
				line.entLink = data[i].entity;
				line.OnSelect = function(line)
					local menu = DermaMenu() 
					local remove = menu:AddOption( "Remove position", function() 
							netstream.Start('jobVendor::removeJob', line.jobID)
					end)
					remove:SetIcon("icon16/bin_closed.png")
					local showPos = menu:AddOption( "Show position for 30 seconds", function()
							HALO_SEARCH = line.entLink;
							local uniqueID = "search the haloed"
							timer.Create(uniqueID, 30, 1, function()
									if !timer.Exists(uniqueID) or !LocalPlayer():IsValid() then timer.Remove(uniqueID) return; end;
									HALO_SEARCH = nil;
							end);
					end)
