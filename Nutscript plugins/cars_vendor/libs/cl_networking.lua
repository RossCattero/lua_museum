netstream.Hook("nut.car_vendor.openUI", function()
...
netstream.Hook("nut.car_vendor.sync", function( car )
	nut.car_vendor.Instance(car.charID, car.uniqueID, NULL)
...
netstream.Hook("nut.car_vendor.remove", function( charID )
	nut.car_vendor.instances[charID] = nil;
end)

netstream.Hook("nut.car_vendor.openBaggage", function(baggageID)
	if nut.inventory.instances[baggageID] then
		CarStorage(nut.inventory.instances[baggageID])
	end;
end)

function CarStorage(storage)
