...
function PANEL:Populate()
		...
		self.name = self.nameSubtitle:Add("DLabel")
		self.name:Dock(FILL)
		self.name:SetAutoStretchVertical(false)
		self.name:SetTextColor(Color(0, 0, 0))
		self.name:SetFont("Typewriter-check")
		self.name:DockMargin(sw * 0.01, 0, 0, 0)
		if CHECK["whoIs"]:len() > 0 then
			local name = CHECK["whoIs"]
		...
		self.bank = self.bankSubtitle:Add("DLabel")
		self.bank:Dock(FILL)
		self.bank:SetAutoStretchVertical(false)
		self.bank:SetTextColor(Color(0,0,0))
		self.bank:SetFont("Typewriter-check")
		self.bank:SetText("Reichbank")
		//
		...
		self.FooterSubtitle.Paint = function(s, w, h)
			local minus = .1
			local offset = 27
			draw.SimpleText("PAY TO THE", "Typewriter-check", offset, 12, Color(0, 0, 0, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
			draw.SimpleText("ORDER OF", "Typewriter-check", offset-1, 25, Color(0, 0, 0, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
			surface.SetDrawColor(90, 90, 90)
		...
		if CHECK["orderFor"]:len() > 0 then self.FooterEntry:SetEnabled(false) end;
		self.FooterEntry:Dock(BOTTOM)
		self.FooterEntry:SetFont("Gbanking-handy")
		self.FooterEntry:SetText(CHECK["orderFor"])
		...
		self.FooterDollarSign.Paint = function(s, w, h)
				draw.SimpleText(BANKING_REICHMARK, "Typewriter-check", w * 0.35, h * 0.2, Color(0, 0, 0, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
		end;
		//
		...
		if CHECK["amount"] > 0 then self.ButtonType:SetEnabled(false) end;
		self.ButtonType:SetText(CHECK["amount"] > 0 && CHECK["amount"] || "")
		self.ButtonType:Dock(FILL)
		self.ButtonType:SetFont("Gbanking-handy")
		...
		self.Memo.Paint = function(s, w, h)
			...
			draw.SimpleText("MEMO", "Typewriter-check", 0, h * 0.5, Color(0, 0, 0, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
		end;
		...
		if CHECK["sign"]:len() > 0 then self.SubScript:SetEnabled(false) end;
		self.SubScript:SetValue(CHECK["sign"]:len() > 0 && CHECK["sign"] || "")
		self.SubScript:Dock(BOTTOM)
		self.SubScript:SetFont("Gbanking-handy")
		...
		self.idPlace = self.FooterNum:Add("DPanel")
		self.idPlace:Dock(LEFT)
		self.idPlace:SetWide(sw * 0.03)
		self.idPlace:DockMargin(0, sh * 0.01, 0, 0)
		self.idPlace.Paint = function(s, w, h)
			draw.RoundedBox(0,0,0, w, h, Color(216, 188, 31))
		end;
		
		self.UniqueIDCheck = self.idPlace:Add("DLabel")
		self.UniqueIDCheck:Dock(FILL)
		self.UniqueIDCheck:SetText(CHECK["checkID"] or "0000")
		self.UniqueIDCheck:SetTextColor(Color(0, 0, 0))
		self.UniqueIDCheck:SetFont("Typewriter-check")
		self.UniqueIDCheck:SetContentAlignment(5)

		self.CloseBTN = self.FooterNum:Add("DLabel")
		...
		self.CloseBTN:SetFont("Typewriter-check")
		...
		self.CloseBTN.DoClick = function(btn)
				...
				if amount && amount > 0 then
						CHECK.amount = amount;
						CHECK.orderFor = self.FooterEntry:GetValue();
						CHECK.sign = self.SubScript:GetValue();

						netstream.Start('Banking::submit', CHECK)
				end
				self:Close()
		end;
...
function PANEL:Paint( w, h )
    ...
    surface.DrawOutlinedRect( 6, 6, w-12, h-12, 1 )
		...
		draw.SimpleText("Payment", "Typewriter-check", w * 0.17, h * 0.67, Color(67, 126, 236), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
end;

vgui.Register( "CheckPanel", PANEL, "SPanel" )
